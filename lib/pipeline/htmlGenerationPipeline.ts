...
// When the prompt describes a shop/store we pre-generate supabase.js (static
// data layer template with real Supabase credentials if available) plus a SQL
// migration with schema + realistic seed data generated by Claude.
...
export async function runGenerationPipeline(
  ...
  const needsConsent = isEcommercePrompt(userPrompt) || !!(resolvedConfig.consentRequired) || !!(resolvedConfig.analyticsConfig);
  if (needsConsent) {
  ...
  // Step 2b: For ecommerce sites, pre-generate supabase.js (static data layer with
  // real Supabase credentials if connected) and a SQL migration with schema + seed data.
  // Both files are emitted BEFORE page generation so pages reference a single
  // cart source-of-truth, eliminating basket-count drift and promo-code bugs.
  ...
  for (let i = 0; i < pages.length; i++) {
    ...
    const pageContent = await generateSinglePage(
      page, siteName, userPrompt, pages, sharedFiles, onProgress, dbConnection, isEcommerce, ecommerceSupabaseJs
    );

          ...
          <div className="flex flex-col min-h-[400px] lg:min-h-0">
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 flex-1 overflow-hidden relative">
              <ErrorBoundary>
                ...
                ) : /* Branch 1c: Next.js multifile, WebContainer off → project-ready landing */
                isMultiFileProject && projectFiles.length > 0 && !useWebContainer ? (
                  <div className="absolute inset-0 flex items-center justify-center bg-gray-950 p-8">
                    <div className="text-center max-w-xs w-full">
                      ...
                      <h2 className="text-lg font-bold text-white mb-1">
                        {projectName || 'Your app'} is ready
                      </h2>
                      <p className="text-sm text-gray-400 mb-6">
                        {projectFiles.length} files · Next.js + Supabase
                      </p>
                      <div className="flex flex-col gap-3">
                        <button
                          onClick={() => setShowCodeViewer(true)}
                          className="flex items-center justify-center gap-2 px-4 py-3 bg-indigo-600 hover:bg-indigo-700 text-white text-sm font-semibold rounded-xl transition-colors"
                        >
                          <Code2 className="w-4 h-4" />
                          Browse Files
                        </button>
                        <button
                          onClick={async () => {
                            ...
                            const filesRecord = Object.fromEntries(projectFiles.map(f => [f.path, f.content]));
                            // Next.js dev server reads .env.local before .env.
                            // Copy injected credentials so they're picked up immediately.
                            if (filesRecord['.env']) {
                              filesRecord['.env.local'] = filesRecord['.env'];
                            }
                            sdk.openProject(
                              { title: projectName || 'Generated App', description: 'Next.js + Supabase app generated by BuildFlow', template: 'node', files: filesRecord },
                              { newWindow: true, startScript: 'dev' }
                            );
                          }}
                          className="flex items-center justify-center gap-2 px-4 py-3 bg-gray-800 hover:bg-gray-700 text-white text-sm font-semibold rounded-xl transition-colors"
                        >
                          <ExternalLink className="w-4 h-4" />
                          Open in StackBlitz
                        </button>
        ...
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <GenerationWizard
            ...
            onOpenInBuilder={() => {
              ...
              // Sync saved project ID so the center panel can link to it
              if (wizardProjectId) setCurrentProjectId(wizardProjectId);
              setShowWizard(false);
              // Auto-open the file viewer so the user immediately sees their code

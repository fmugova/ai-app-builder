...
// ── CIDR-based private IP blocking (fixes CodeQL alert #59: DNS rebinding) ──
...
const PRIVATE_CIDRS = [
  ...
  { base: '169.254.0.0', mask: 16 }, // link-local (APIPA)
  { base: '100.64.0.0',  mask: 10 }, // shared address (CGNAT)
 ...
 * Blocks DNS rebinding attacks where a name resolves to a private address.
 */
...
function isValidUrlInput(url: string): boolean {
  ...
  // Block trivially encoded localhost/loopback variants before DNS resolution
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!isValidUrlInput(url)) {
      console.warn(`[Suspicious Request] User: ${session.user.id} | Invalid URL: ${String(url).slice(0, 200)}`)
    ...
    try {
      ...
      console.warn(`[Malformed URL] User: ${session.user.id} | URL: ${String(url).slice(0, 200)}`)
    ...
    // ── DNS-based CIDR check — prevents IPv6 & DNS-rebinding SSRF bypasses ───
    const isSafeHost = await isSafePublicHost(hostname)
    if (!isSafeHost) {
      console.warn(`[Blocked Private IP Range] User: ${session.user.id} | Hostname: ${hostname}`)
      return NextResponse.json(
        { error: 'Access to private/local IP ranges is not allowed' },
        { status: 403 }
      )
    }
    ...
    if (!response.ok) {
      console.error(
        `[Fetch Failed] User: ${session.user.id} | Status: ${response.status} ${response.statusText} | URL: ${url}`
      )
      return NextResponse.json(
